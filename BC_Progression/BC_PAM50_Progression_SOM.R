...
for (i in seq(1,3)){
  BC_GG_pathdata[[i]]<-read.xlsx(xlsxFile = 'BC_PAM50_pseudotime_data.xlsx',
                            colNames = FALSE,
                            rowNames = TRUE,
                            sheet = i)
  names(BC_GG_pathdata)[i]<-paste('Path',i,sep='_')
  BC_GG_pseudotime[[3-(i-1)]]<-read.xlsx(xlsxFile = 'BC_PAM50_pseudotime.xlsx',
                                 colNames = FALSE,
                                 sheet = i)
...
for (p in 2:length(BC_GG_pathdata)){
  #Extract data
  ...
  plotList<-lapply(unique(genesom$unit.classif)[order(unique(genesom$unit.classif))],function(x){
    ...
    p<-ggplot(dta_melt,aes(x=pseudotime,y=value,colour=gene)) + 
      ...
      theme(axis.text=element_text(size=7),axis.title = element_text(size=9),
            ...
            legend.position = 'bottom',legend.margin = margin(t=-25,l=-10))
    return(p)
  })
  #Make the grid:
  # layout_matrix<-ifelse(length(plotList)<9,matrix(seq(1,6),nrow=3,ncol = 2,byrow = TRUE),
  #                       matrix(seq(1,9),nrow=3,ncol = 3,byrow = TRUE))
...
pam50_pseudotime_normal_to_luminal<-pathPlot_rawPlots[['Path_2']];save(file='pam50_pseudotime_normal_to_luminal.rda',pam50_pseudotime_normal_to_luminal)
pam50_pseudotime_normal_to_basal<-pathPlot_rawPlots[['Path_3']];save(file='pam50_pseudotime_normal_to_basal.rda',pam50_pseudotime_normal_to_basal)
ggsave(filename = 'NormalLuminal_PAM50_SOMNodes.jpg',plot = pathPlot_objs[["Path_2"]],width = 8.5,height=8.5,units = 'in')
ggsave(filename = 'NormalBasal_PAM50_SOMNodes.jpg',plot = pathPlot_objs[["Path_3"]],width = 8.5,height=8.5,units = 'in')
save(file='BC_PAM50_sigcor_path_glycogenes.rda',sigcor_pathgenes)